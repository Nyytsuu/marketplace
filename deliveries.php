...
if (isset($_POST['logout'])) {
    session_unset();
    session_destroy();
    header("Location: driver.php"); // Fixed: Changed from ../driver.php to driver.php
    exit();
}
?>
...
document.getElementById('logout-btn').addEventListener('click', function () {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      window.location.href = 'driver-logout.php';
    }
  });
});
</script>
        ...
        <?php if (empty($driverArea['delivery_barangay']) || empty($driverArea['delivery_city']) || empty($driverArea['delivery_province'])): ?>
          ...
          <div class="deliveries-section">
            ...
            <form method="POST" id="areaForm">
              ...
              <button type="submit" class="btn btn-submit">Save Area</button>
            </form>
          </div>
        <?php else: ?>
          ...
          <div id="areaModal" class="modal">
            <div class="modal-content">
              ...
              <form id="areaFormModal" method="POST">
                ...
                <div class="form-group">
                  <label for="provinceModal">Province:</label>
                ...
                <div class="btn-group">
                  <button type="button" id="cancelModal" class="btn btn-cancel">Cancel</button>
                  <button type="submit" class="btn btn-submit">Save Area</button>
                </div>
              </form>
            </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function setupLocationSelectors(provinceSelect, citySelect, barangaySelect) {
    // Use the location API file in the same directory
    const baseURL = "location_api.php";

    // Load provinces with error handling
    fetch(`${baseURL}?type=province`)
      ...
      .catch(error => {
        ...
        // Add some default provinces if API fails
        const defaultProvinces = ['Metro Manila', 'Cavite', 'Laguna', 'Batangas', 'Rizal'];
    ...
    provinceSelect.addEventListener("change", () => {
      ...
      fetch(`${baseURL}?type=city&province=${encodeURIComponent(province)}`)
        ...
        .catch(error => {
          ...
          // Add default cities based on province
          let defaultCities = [];
          if (province === 'Metro Manila') {
            defaultCities = ['Manila', 'Quezon City', 'Makati', 'Pasig', 'Taguig'];
          } else {
            defaultCities = [province + ' City', 'Other City'];
          }
      ...
      if (province && city) {
        fetch(`${baseURL}?type=barangay&province=${encodeURIComponent(province)}&city=${encodeURIComponent(city)}`)
          ...
          .catch(error => {
            ...
            const defaultBarangays = ['Barangay 1', 'Barangay 2', 'Barangay 3', 'Barangay 4', 'Barangay 5'];
            defaultBarangays.forEach(barangay => {
              const opt = document.createElement("option");
              opt.value = barangay;
              opt.textContent = barangay;
              barangaySelect.appendChild(opt);
